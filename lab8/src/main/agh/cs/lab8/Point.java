...
public class Point {
    public static NodePair parsePoint(int i, ArrayList<String> file, String line) {
        ...
        i++;
        for (; !file.get(i).matches("^\\d+[)] .*") && !file.get(i).matches("^\\d+[.] .*") && !file.get(i).matches("^Art[.] .*") && !file.get(i).matches("^Rozdział.*") && !file.get(i).matches("^DZIAŁ.*"); i++) {
            node.addContent(file.get(i));
        }

        NodePair tmp;
        for (; i<file.size() && !file.get(i).matches("^\\d+[.] .*") && !file.get(i).matches("^Art[.] .*") && !file.get(i).matches("^Rozdział.*") && !file.get(i).matches("^DZIAŁ.*");) {
